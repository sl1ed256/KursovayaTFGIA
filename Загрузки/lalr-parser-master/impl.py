...
def term_and_nonterm(grammar,term,non_term):#функция вычисления терминальных и нетерминальных символов
    for prod in grammar:
...
def calculate_first(grammar,first,term,non_term):#вычисление First
    for t in term:
        first[t] = t;
    for nt in non_term:
        first[nt] = set({})
...
def get_first(nt,grammar,first,term):#вычисление First
    for prod in grammar:
        if nt in prod[0]:
            rhs = prod[1]
            first_char = rhs[0]
...
def closure(I,augment_grammar,first,non_term):#вычисление Closure
    while True:
                ...
                for prod in augment_grammar:
                    if next_char == prod[0]:
                        if prod[1] == 'e':
                            ...
                            rhs = '.' + prod[1]
                        la = []                                     #просмотр следующих символов (look ahead)
                        ...
                        new_item = [next_char,rhs,la]               #структура нового состояния
...
def goto(I,X,augment_grammar,first,non_term): #перевод точки после X(терминал или нетерминал грамматики)
    J =[]
    for item in I:
        cursor_pos = item[1].index('.')
        if cursor_pos < len(item[1])-1:
...
def get_parse_table(parse_table,states,augmented_grammar):                      #Lr состояния преобразуются в Lalr
    ambiguous = False
    ...
    if ambiguous:
        print("Неоднозначная грамматика!\n\n")
